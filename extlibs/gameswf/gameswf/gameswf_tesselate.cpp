...
namespace tesselate
{
	...
	// More Renderer state.
	static tu_array<fill_segment>	s_current_segments;	// @@ should not dynamically resize this thing!
	static tu_array<point>	s_current_path;			// @@ should not dynamically resize this thing!
	...
	// trapezoids.  Modifies the values in s_current_segments.
	{
		...
		// Peel off first.
		tu_array<fill_segment>	slab;	// @@ make this use static storage
		for (int i = i0; i < i1; i++)
		{
...
namespace tesselate_new
{
	...
	struct path_part
	{
		...
		tu_array<point> m_verts;
	};


	static tu_array<path_part>	s_path_parts;
	static point	s_last_point;
	...
	void copy_points_into_array(tu_array<float>* out, const tu_array<point>& in)
	{
		if (in.size() > 0) {
