...
//			assert(index >= 0 && index < m_stack_size);
			return tu_array<as_value>::operator[](index);
		}
...
//			assert(index >= 0 && index < m_stack_size);
		...
		void	push(T val) 
		{
			if (m_stack_size < tu_array<as_value>::size())
			{
...
//				(*this)[m_stack_size] = as_value(val);
	...
	struct as_environment : public vm_stack
	{
		...
		tu_array<as_value>	m_local_register;	// function2 uses this
		...
		tu_array<frame_slot>	m_local_frames;
		...
		as_value	get_variable(const tu_string& varname, const tu_array<with_stack_entry>& with_stack) const;
		// no path stuff:
		as_value	get_variable_raw(const tu_string& varname, const tu_array<with_stack_entry>& with_stack) const;

		void	set_variable(const tu_string& path, const as_value& val, const tu_array<with_stack_entry>& with_stack);
		// no path stuff:
		void	set_variable_raw(const tu_string& path, const as_value& val, const tu_array<with_stack_entry>& with_stack);
