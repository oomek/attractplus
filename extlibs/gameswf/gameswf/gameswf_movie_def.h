...
//				printf("signal for frame %d\n", m_loading_frame);
		...
		virtual const tu_array<execute_tag*>&	get_playlist(int frame_number) = 0;
		virtual const tu_array<execute_tag*>*	get_init_actions(int frame_number) = 0;
	...
	struct movie_def_impl : public movie_definition_sub
	{
		...
		tu_array<tu_array<execute_tag*> >	   m_playlist;	// A list of movie control events for each frame.
		tu_array<tu_array<execute_tag*> >	   m_init_action_list;	// Init actions for each frame.
		stringi_hash<int>		   m_named_frames;	// 0-based frame #'s
		stringi_hash<gc_ptr<character_def> > m_exports;

		// Items we import.
		tu_array<import_info>	m_imports;

		// Movies we import from; hold a ref on these, to keep them alive
		tu_array<gc_ptr<movie_definition> >	m_import_source_movies;

		// Bitmaps used in this movie; collected in one place to make
		// it possible for the host to manage them as textures.
		tu_array<gc_ptr<bitmap_info> >	m_bitmap_list;
		...
		virtual const tu_array<execute_tag*>&	get_playlist(int frame_number);

		virtual const tu_array<execute_tag*>*	get_init_actions(int frame_number);
		void	read(tu_file* in);
		void	read_tags();
		void	get_owned_fonts(tu_array<font*>* fonts);
