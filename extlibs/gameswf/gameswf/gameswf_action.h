...
namespace gameswf
{
	...
	struct event_id
	{
		...
		event_id(id_code id, tu_array<as_value>* args) :
	...
	struct action_buffer
	{
		...
		void	execute(
			...
			int exec_bytes,
			as_value* retval,
			const tu_array<with_stack_entry>& initial_with_stack,
			bool is_function2) const;

		static as_object* load_as_plugin(player* player,
			const tu_string& classname, const tu_array<as_value>& params);
	...
	private:
		// Don't put these as values in tu_array<>!  They contain
		...
		tu_array<tu_string>	m_dictionary;
