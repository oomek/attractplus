...
namespace gameswf
{
	...
	private:
		...
		tu_array<gc_ptr<shape_character_def> >	m_glyphs;
		...
		float	m_descent;
		float	m_leading;
		tu_array<float>	m_advance_table;
		// @@ we don't seem to use this thing at all, so don't bother keeping it.
		// tu_array<rect>	m_bounds_table;	// @@ this thing should be optional.
		...
		struct zone_record
		{
			...
			tu_array<zone_data> m_zone_data;

			bool m_has_maskx;
			bool m_has_masky;
		};

		tu_array<zone_record> m_zone_table;
	};
