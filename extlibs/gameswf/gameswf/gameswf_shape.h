...
namespace gameswf
{
	...
	private:
		tu_array<coord_component>	m_triangle_strip;// TODO remove
		tu_array<coord_component> m_triangle_list;
	};
	...
	private:
		int	m_style;
		tu_array<coord_component>	m_coords;
	};
	...
	// certain error tolerance.
	{
		...
		void display( const matrix& m, const cxform& cx, const tu_array<fill_style>& fills,
	...
	private:
		...
		struct layer {
			tu_array<mesh*> m_meshes;  // one mesh per style.
			tu_array<line_strip*> m_line_strips;

			~layer();  // delete m_meshes & m_line_strips
		};
		tu_array<layer> m_layers;
	};
	...
	// information on fill and line styles.
	{
		...
		void	display( const matrix& mat, const cxform& cx, float pixel_scale, const tu_array<fill_style>& fill_styles,
				const tu_array<line_style>& line_styles, render_handler::bitmap_blend_mode bm) const;
		...
		const tu_array<fill_style>&	get_fill_styles() const { return m_fill_styles; }
		const tu_array<line_style>&	get_line_styles() const { return m_line_styles; }
		const tu_array<path>&	get_paths() const { return m_paths; }
	...
	protected:
		...
		tu_array<fill_style>	m_fill_styles;
		tu_array<line_style>	m_line_styles;
		tu_array<path>	m_paths;
	...
	private:
		...
		mutable tu_array<mesh_set*>	m_cached_meshes;
	};
