...
namespace gameswf
{
	...
	const tu_array<execute_tag*>&	movie_def_impl::get_playlist(int frame_number) { return m_playlist[frame_number]; }

	const tu_array<execute_tag*>*	movie_def_impl::get_init_actions(int frame_number)
	{
	...
	void	movie_def_impl::get_owned_fonts(tu_array<font*>* fonts)
	// Fill up *fonts with fonts that we own.
	{
		assert(fonts);
		fonts->resize(0);

		tu_array<int>	font_ids;
