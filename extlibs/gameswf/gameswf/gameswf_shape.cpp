...
namespace gameswf
{
	...
	void	write_coord_array(tu_file* out, const tu_array<coord_component>& pt_array)
	...
	void	read_coord_array(tu_file* in, tu_array<coord_component>* pt_array)
...
//		memcpy(&m_coords[0], coords, coord_count * sizeof(coords[0]));
	...
	struct tri_stripper
	{
		...
		tu_array< tu_array<point> >	m_strips;
		...
		// under the given style.
		{
			if (m_strips.size())
			{
				...
				for (int i = 1, n = m_strips.size(); i < n; i++)
				{
					...
					const tu_array<point>&	str = m_strips[i];
					assert(str.size() >= 3);	// should have at least one tri already.
	...
	void	mesh_set::display(
		const matrix& mat,
		const cxform& cx,
		const tu_array<fill_style>& fills,
		const tu_array<line_style>& line_styles, render_handler::bitmap_blend_mode bm) const
	...
	static void	read_fill_styles(tu_array<fill_style>* styles, stream* in, int tag_type, movie_definition_sub* m)
	...
	static void	read_line_styles(tu_array<line_style>* styles, stream* in, int tag_type, movie_definition_sub* m)
...
#define SHAPE_LOG 0
	...
	static void	debug_display_shape_paths(
		...
		const tu_array<path>& paths,
		const tu_array<fill_style>& fill_styles,
		const tu_array<line_style>& line_styles)
	{
...
// 		// xxxxxxx
	...
	void	shape_character_def::display( const matrix& mat, const cxform& cx, float pixel_scale, const tu_array<fill_style>& fill_styles, const tu_array<line_style>& line_styles, render_handler::bitmap_blend_mode bm) const
