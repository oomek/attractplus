...
namespace gameswf
{
	...
	{
		assert(new_size <= tu_array<as_value>::size());
	...
	void	vm_stack::drop(int count)
	{
		...
		for (int i = m_stack_size, n = tu_array<as_value>::size(); i < n; i++)
		{
	...
	{
		for (int i = 0; i < tu_array<as_value>::size(); i++)
		{
	...
	as_value	as_environment::get_variable(const tu_string& varname, const tu_array<with_stack_entry>& with_stack) const
	...
	as_value	as_environment::get_variable_raw(
		...
		const tu_array<with_stack_entry>& with_stack) const
	...
	void	as_environment::set_variable(
		const tu_string& varname,
		const as_value& val,
		const tu_array<with_stack_entry>& with_stack)
