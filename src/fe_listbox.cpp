...
void FeListBox::init_dimensions()
{
	...
	for ( int i=0; i< m_rows; i++ )
	{
		...
		int selection_row = getSelectionRow();
...
void FeListBox::setSelColor( sf::Color c )
{
	...
	if ( m_texts.size() > 0 )
	{
		...
		if ( selection_row >= 0 && selection_row < (int)m_texts.size() )
			m_texts[ selection_row ].setColor( m_selColour );
	}
...
void FeListBox::setSelBgColor( sf::Color c )
{
	...
	if ( m_texts.size() > 0 )
	{
		...
		if ( selection_row >= 0 && selection_row < (int)m_texts.size() )
			m_texts[ selection_row ].setBgColor( m_selBg );
	}
...
void FeListBox::setSelStyle( int s )
{
	...
	if ( m_texts.size() > 0 )
	{
		...
		if ( selection_row >= 0 && selection_row < (int)m_texts.size() )
			m_texts[ selection_row ].setStyle( m_selStyle );
	}
...
FeTextPrimitive *FeListBox::getMiddleText()
{
	...
	if ( selection_row >= 0 && selection_row < (int)m_texts.size() )
		return &m_texts[ selection_row ];
	else
		return &m_texts[ m_texts.size() / 2 ];  // Fallback to cenre
}
...
void FeListBox::internalSetText( const int index )
{
	...
	switch ( m_mode )
	{
		case Moving:
		{
			...
			if ( margin >= display_rows ) margin = display_rows - 1;
			...
			else
			{
				if ( index < window_start + margin )
				{
					...
					m_selection_pos = index - m_list_start_offset;
				}
				else if (index > window_end - margin)
				{
					// Scroll the list down
					m_list_start_offset = std::min( list_size - display_rows, index - ( display_rows - 1 - margin ));
					m_selection_pos = index - m_list_start_offset;
				}
				else
				{
		...
		case Paged:
		{
			...
			if ( list_size <= display_rows )
			{
				m_selection_pos = index % display_rows;
				m_list_start_offset = 0;
			}
			else
			{
		...
		default: // Fallback to Static mode
			m_selection_pos = m_texts.size() / 2;
			offset = index - m_selection_pos;
...
int FeListBox::getSelectionRow() const
{
	if ( m_mode == Static )
		return m_texts.size() / 2;
	else
