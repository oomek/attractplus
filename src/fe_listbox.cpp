...
void FeListBox::internalSetText( const int index )
{
	...
	for ( int i=0; i < display_rows; i++ )
	{
		...
		if ( listentry < 0 || listentry >= list_size )
			...
			m_texts[i].setString( m_displayList[ listentry % list_size ] );
	}
...
void FeListBox::on_new_list( FeSettings *s )
{
	...
	int display_index = s->get_current_display_index();
	...
	// Processing magic strings only if display/filter has changed
	if ( m_displayList_display_index != display_index || m_displayList_filter_index != filter_index )
	{
		std::string format_string = m_format_string.empty()
			? DEFAULT_FORMAT_STRING
			: m_format_string;

		m_displayList_display_index = display_index;
		m_displayList_filter_index = filter_index;
