...
size_t FeZipStream::read( void *data, size_t size )
{
	if ( m_data.empty() )
		return -1;

	size_t end_pos = m_pos + size;
	size_t count = ( end_pos <= (size_t)m_data.size() )
...
size_t FeZipStream::seek( size_t position )
{
	if ( m_data.empty() )
		return -1;

	m_pos = ( position < (size_t)m_data.size() ) ? position : m_data.size();
	return m_pos;
}

size_t FeZipStream::tell()
{
	if ( m_data.empty() )
		return -1;
