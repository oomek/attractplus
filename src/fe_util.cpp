...
const int CONFIG_NAME_WIDTH = 6 * TAB_SIZE;

// Add simple quoting if value contains a space
...
// Write an indent using spaces
void write_indent( nowide::ofstream &f, int indent )
{
	if ( indent )
		f << std::string( TAB_SIZE * indent, ' ');
}

// Write a "Generated by" header
void write_header( nowide::ofstream &f )
{
	f << "# Generated by " << FE_NAME << " " << FE_VERSION << std::endl;
	f << "#" << std::endl;
}
...
// Write a key/value pair, any number of spaces are allowed between
void write_pair( nowide::ofstream &f, std::string name, std::string value, int indent )
{
	write_indent( f, indent );
	if ( !value.empty() )
		f << std::setw(CONFIG_NAME_WIDTH) << std::left << ( name + " " ) << value << std::endl;
	else
		f << name << std::endl;
}

// Write a type/key/value combination
// - The following behavior must be followed for legacy configs to work
// - All "param" type values require a SINGLE SPACE between the name and value
// - The value is printed RAW, and may contain leading spaces, quotes, etc
void write_param( nowide::ofstream &f, std::string param, std::string name, std::string value, int indent )
{
	write_indent( f, indent );
	if ( !value.empty() )
		f << std::setw(CONFIG_NAME_WIDTH) << std::left << ( param + " " + name + " " ) << value << std::endl;
	else
...
namespace
{
...

	{
		...
		while (( hotkey.get_current_state( opt->joy_thresh ) )
			&& ( t.getElapsedTime() < fe_milliseconds( TIMEOUT_MS ) ))
		{
			if ( opt->wait_cb )
				opt->wait_cb( opt->launch_opaque );

			fe_sleep( fe_milliseconds( 10 ) );
		}
		...
		if ( w == 0 )
		{
			...
			if ( process_check_for_hotkey( opt, exit_is ) )
			{
				...
				if ( kill( pid, 0 ) == 0 )
				{
					...
					while (( term_clock.getElapsedTime() < fe_milliseconds( TERM_TIMEOUT ) )
						&& ( waitpid( pid, &status, WNOHANG ) == 0 ))
					{
						fe_sleep( fe_milliseconds( POLL_FOR_EXIT_MS ) );
					}
