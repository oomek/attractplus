...
namespace
{
	...
	std::string build_pbr_fragment_shader()
	{
		return
			...
			"\tfloat upper_lod = lower_lod + 1.0;\n"
			"\tfloat lod_blend = fract( lod );\n"
			"\tvec3 R = normalize( direction );\n"
			"\tvec3 lower = srgb_to_linear( textureLod( cubemap_texture, R, lower_lod ).rgb );\n"
			"\tvec3 upper = srgb_to_linear( textureLod( cubemap_texture, R, upper_lod ).rgb );\n"
			"\treturn mix( lower, upper, lod_blend );\n"
			"}\n"
			"float cubemap_roughness_lod( float roughness, float max_lod )\n"
			"{\n"
			"\tfloat value = clamp( roughness, 0.0, 1.0 );\n"
			"\tvalue = value + value * ( 1.0 - value ) * 0.45;\n"
			"\treturn value * max_lod;\n"
			"}\n"
			"vec3 sample_cubemap_specular( vec3 direction, float roughness, float max_lod )\n"
			"{\n"
			"\tvec3 R = normalize( direction );\n"
			"\tfloat lod = cubemap_roughness_lod( roughness, max_lod );\n"
			...
			"\tfloat anisotropic_weight = 1.0 - smoothstep( 0.06, 0.24, roughness );\n"
			...
			"\t\tvec3 L = vec3( 0.0 );\n"
			"\t\tfloat area_specular_weight = 0.0;\n"
			"\t\tfloat area_specular_mask = 0.0;\n"
			"\t\tfloat area_specular_energy = 1.0;\n"
			...
			"\t\t\tif ( light_radius > 0.0 )\n"
			"\t\t\t{\n"
			"\t\t\t\tfloat angular_radius = clamp( light_radius / distance_to_light, 0.0, 1.0 );\n"
			"\t\t\t\tfloat material_lobe = max( roughness * roughness, 0.0001 );\n"
			"\t\t\t\tfloat light_lobe = angular_radius * angular_radius;\n"
			"\t\t\t\tfloat blend_lobe = max( material_lobe * material_lobe, 0.0001 );\n"
			"\t\t\t\tarea_specular_weight = smoothstep( 0.0, 1.0, clamp( light_lobe / max( blend_lobe + light_lobe, 0.0001 ), 0.0, 1.0 ) );\n"
			"\t\t\t\tarea_specular_energy = material_lobe / max( material_lobe + light_lobe, 0.0001 );\n"
			"\t\t\t\tvec3 R = reflect( -V, N );\n"
			"\t\t\t\tfloat specular_cos = clamp( dot( R, L ), -1.0, 1.0 );\n"
			"\t\t\t\tfloat specular_sin = specular_cos > 0.0 ? sqrt( max( 1.0 - specular_cos * specular_cos, 0.0 ) ) : 1.0;\n"
			"\t\t\t\tfloat edge_width = max( 0.01, roughness * 0.25 );\n"
			"\t\t\t\tarea_specular_mask = 1.0 - smoothstep( max( angular_radius - edge_width, 0.0 ), angular_radius + edge_width, specular_sin );\n"
			"\t\t\t}\n"
			...
			"\t\tvec3 point_specular = numerator / denominator;\n"
			"\t\tfloat area_gloss = 1.0 - roughness;\n"
			"\t\tarea_gloss *= area_gloss;\n"
			"\t\tvec3 area_specular = F * area_specular_mask * area_specular_energy * area_gloss * AREA_SPECULAR_SCALE;\n"
			"\t\tvec3 specular = mix( point_specular, area_specular, area_specular_weight );\n"
			"\t\tvec3 kS = F;\n"
