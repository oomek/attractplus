...
namespace
{
	...
	std::string build_pbr_fragment_shader()
	{
		return
			...
			"}\n"
			"vec2 transform_uv( vec2 uv, int index, out bool inside_fit )\n"
			"{\n"
			...
			"\t\tinside_fit =\n"
			"\t\t\tuv.x >= fit_offset.x && uv.x <= ( fit_offset.x + fit_scale.x ) &&\n"
			"\t\t\tuv.y >= fit_offset.y && uv.y <= ( fit_offset.y + fit_scale.y );\n"
			"\t\tif ( inside_fit )\n"
			"\t\t\tuv = ( uv - fit_offset ) / fit_scale;\n"
			"\t}\n"
			"\tuv *= offset_scale.zw;\n"
			...
			"{\n"
			"\tbool inside_fit = true;\n"
			"\tvec2 uv = transform_uv( select_uv( pbr.transforms_rotation_texcoord[0].y ), 0, inside_fit );\n"
			"\tif ( !inside_fit )\n"
			"\t\treturn vec4( 0.0, 0.0, 0.0, 1.0 );\n"
			"\treturn texture( base_color_texture, uv );\n"
			"}\n"
			"vec4 sample_metallic_roughness()\n"
			"{\n"
			"\tbool inside_fit = true;\n"
			"\treturn texture( metallic_roughness_texture, transform_uv( select_uv( pbr.transforms_rotation_texcoord[1].y ), 1, inside_fit ) );\n"
			"}\n"
			"vec3 sample_normal_tex()\n"
			"{\n"
			"\tbool inside_fit = true;\n"
			"\treturn texture( normal_texture, transform_uv( select_uv( pbr.transforms_rotation_texcoord[2].y ), 2, inside_fit ) ).xyz;\n"
			"}\n"
			"float sample_occlusion()\n"
			"{\n"
			"\tbool inside_fit = true;\n"
			"\treturn texture( occlusion_texture, transform_uv( select_uv( pbr.transforms_rotation_texcoord[3].y ), 3, inside_fit ) ).r;\n"
			"}\n"
			"vec3 sample_emissive()\n"
			"{\n"
			"\tbool inside_fit = true;\n"
			"\tvec2 uv = transform_uv( select_uv( pbr.transforms_rotation_texcoord[4].y ), 4, inside_fit );\n"
			"\tif ( !inside_fit )\n"
			"\t\treturn vec3( 0.0 );\n"
