...
float FeSound::get_x()
{
	return m_sounds.back().getPosition().x;
}

float FeSound::get_y()
{
	return m_sounds.back().getPosition().y;
}

float FeSound::get_z()
{
	return m_sounds.back().getPosition().z;
}

void FeSound::set_x( float v )
{
	m_sounds.back().setPosition( sf::Vector3f( v, get_y(), get_z() ) );
}

void FeSound::set_y( float v )
{
	m_sounds.back().setPosition( sf::Vector3f( get_x(), v, get_z() ) );
}

void FeSound::set_z( float v )
{
	m_sounds.back().setPosition( sf::Vector3f( get_x(), get_y(), v ) );
}
