...
namespace
{
	...
	struct FeAnimationProperty
	{
		std::string name;
		const std::type_info *owner;
		FeAnimation::PropertyMatcher matches;
		FeAnimation::PropertyGetter get;
		FeAnimation::PropertySetter set;
	};
	...
	struct FeAnimationState
	{
		...
		const FeAnimationProperty *property;
		FeEaseFunc ease;
		int ease_id;
		Sqrat::Object obj;
		std::string slot;
	...
	std::vector<FeAnimationProperty> &properties()
	{
		static std::vector<FeAnimationProperty> list;
		return list;
	}
	...
	const FeAnimationProperty *find_property(
		const char *name,
		FeBasePresentable *drawable,
		bool &name_found )
	{
		name_found = false;
		const FeAnimationProperty *match = NULL;

		for ( const FeAnimationProperty &property : properties() )
		{
	...
	void prepare_animated_property( FeBasePresentable *drawable, const FeAnimationProperty *property )
	{
		...
		const FeAnimationProperty *property )
	{
		...
		float anim_val = drawable->snap_destination_to_pixels( property->name, prop_val );
	...
	void stop_animation( FeAnimationState &animation )
	{
		...
		float anim_val = animation.drawable->snap_destination_to_pixels( animation.property->name, prop_val );
	...
	int start_animation( FeAnimationState &animation, float prop_to_val )
	{
		...
		{
			anim_start_val = animation.drawable->snap_destination_to_pixels( animation.property->name, prop_start_val );
			anim_base_start_val = anim_start_val;
		}

		float anim_to_val = animation.drawable->snap_destination_to_pixels( animation.property->name, prop_to_val );
...
FeAnimationObject::FeAnimationObject( int id )
	: m_id( id ),
	m_to( 0.0f ),
	m_duration( 1000.0f ),
	m_time( 0.0f ),
...
float FeAnimationObject::get_duration() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->duration_ms : m_duration;
}

void FeAnimationObject::set_duration( float value )
{
	if ( !std::isfinite( value ))
		return;
...
float FeAnimationObject::get_time() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->time_ms : m_time;
}

void FeAnimationObject::set_time( float value )
{
	if ( !std::isfinite( value ))
		return;
...
int FeAnimationObject::get_ease() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->ease_id : m_ease;
}

void FeAnimationObject::set_ease( int value )
{
...
float FeAnimationObject::get_mass() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->mass : m_mass;
}

void FeAnimationObject::set_mass( float value )
{
	if ( !std::isfinite( value ))
		return;
...
float FeAnimationObject::get_period() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->period : m_period;
}

void FeAnimationObject::set_period( float value )
{
	if ( !std::isfinite( value ))
		return;
...
float FeAnimationObject::get_amplitude() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->amplitude : m_amplitude;
}

void FeAnimationObject::set_amplitude( float value )
{
	if ( !std::isfinite( value ))
		return;
...
float FeAnimationObject::get_strength() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->strength : m_strength;
}

void FeAnimationObject::set_strength( float value )
{
	if ( !std::isfinite( value ))
		return;
...
float FeAnimationObject::get_x1() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->x1 : m_x1;
}

void FeAnimationObject::set_x1( float value )
{
	if ( !std::isfinite( value ))
		return;
...
float FeAnimationObject::get_y1() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->y1 : m_y1;
}

void FeAnimationObject::set_y1( float value )
{
	if ( !std::isfinite( value ))
		return;
...
float FeAnimationObject::get_x2() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->x2 : m_x2;
}

void FeAnimationObject::set_x2( float value )
{
	if ( !std::isfinite( value ))
		return;
...
float FeAnimationObject::get_y2() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->y2 : m_y2;
}

void FeAnimationObject::set_y2( float value )
{
	if ( !std::isfinite( value ))
		return;
...
float FeAnimationObject::get_steps() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->steps : m_steps;
}

void FeAnimationObject::set_steps( float value )
{
	if ( !std::isfinite( value ))
		return;
...
int FeAnimationObject::get_jump() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->jump : m_jump;
}

void FeAnimationObject::set_jump( int value )
{
...
int FeAnimationObject::get_direction() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->direction : m_direction;
}

void FeAnimationObject::set_direction( int value )
{
...
float FeAnimationObject::get_play_count() const
{
	FeAnimationState *animation = find_animation( m_id );
	return animation ? animation->play_count : m_play_count;
}

void FeAnimationObject::set_play_count( float value )
{
...
bool FeAnimationObject::get_running() const
{
	FeAnimationState *animation = find_animation( m_id );
...
void FeAnimation::register_property(
	...
	PropertySetter set )
{
	std::vector<FeAnimationProperty> &list = properties();
	for ( FeAnimationProperty &property : list )
	{
...
SQInteger FeAnimation::script_move( HSQUIRRELVM vm )
{
	...
	const FeAnimationProperty *property = find_property( property_name, drawable, property_name_found );
	if ( !property )
	{
		std::string message;
	...
	{
		Sqrat::ClassType<FeAnimationObject>::PushInstanceCopy( vm, FeAnimationObject( animation.id ) );
		return 1;
	}
	...
	Sqrat::ClassType<FeAnimationObject>::PushInstanceCopy( vm, FeAnimationObject( result_id ) );
	return 1;
}

bool FeAnimation::tick()
{
	std::vector<FeAnimationState> &list = animations();
	if ( list.empty() )
		return false;
	...
	for ( std::vector<FeAnimationState>::size_type i=0; i < list.size(); )
	{
		...
		animation.anim_to_val = animation.drawable->snap_destination_to_pixels(
...
void FeAnimation::remove( FeBasePresentable *drawable, const SQChar *property_name )
{
	std::vector<FeAnimationState> &list = animations();
	list.erase(
		std::remove_if(
