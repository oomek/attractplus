...
-  `w` - The width of the image (in layout coordinates). Default value is `0`, which enables `auto_width`, see [Notes](#artwork-notes).
-  `h` - The height of the image (in layout coordinates). Default value is `0`, which enables `auto_height`, see [Notes](#artwork-notes).
...
-  `width` - Get/set the width of the image (in layout coordinates). Default value is `0`, which enables `auto_width`, see [Notes](#artwork-notes).
-  `height` - Get/set the height of the image (in layout coordinates). Default value is `0`, which enables `auto_height`, see [Notes](#artwork-notes).
-  `auto_width` - Get/set if using the current texture width. Default `true` when `width` is init as `0`.
-  `auto_height` - Get/set if using the current texture height. Default `true` when `height` is init as `0`.
...
- Setting `width` or `height` to `0` *prior* to first-draw (ie: before a tick has occurred) will flag the corresponding `auto_width` or `auto_height` as `true`. Setting them *after* first-draw will simply apply the given value.
-  Setting a negative `width` or `height` will flip the image about its anchor point. To flip an image in-place `subimg` properties can be used:
   ```squirrel
   // flip img vertically
